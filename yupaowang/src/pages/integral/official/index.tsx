...
export default function Integral() {
  ...
  // 初始化页面信息-积分来源记录
  const initType: string = 'official'
  ...
  // 定义列表接收容器
  const [lists, setLists] = useState<IntegralListConfig<IntegralSourceItem>>({
    lists: [],
    hasmore: true
  })
  ...
  useEffect(() => {
    ...
    getIntegralList<IntegralSourceItem>(data).then(res => {
        ...
        {lists.lists.map(item => (
          <View className='integral-list'>
            ...
            <View className='integral-list-item'>
              <View className='integral-list-title overwords'>{item.source_title}</View>
              <View className='integral-list-words overwords'>{item.ext}</View>
              <View className='integral-item-time'>时间：{item.His}<Text>{item.source_integral_string}</Text></View>
            </View>
          </View>
        ))}
      </View>
