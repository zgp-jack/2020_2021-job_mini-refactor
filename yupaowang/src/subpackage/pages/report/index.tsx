...
import { SERVERPHONE, SHOWSERVERPHONE } from '../../../config'
import './index.scss'
    ...
    <View className='yupao-common-container'>
      <View className='report-container'>
        <View className='report-words-box'>
          <View className='report-words-item'>鱼泡网为给工友们提供一个更加安全可靠、方便快捷的招工找活平台，同时也希望为净化建筑行业出一份力
          {SHOWSERVERPHONE &&
          <Block>
          ，故开通举报热线：<Text className='report-words-item-text' onClick={() => { handleClick()}}>{SERVERPHONE }</Text>
          </Block>}
          </View>
