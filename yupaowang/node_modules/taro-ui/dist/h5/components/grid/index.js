...
export default class AtGrid extends AtComponent {
  ...
  render() {
    ...
    return <View className={classNames('at-grid', this.props.className)}>
        {gridGroup.map((item, i) => <View className="at-grid__flex" key={`at-grid-group-${i}`}>
            {item.map((childItem, index) => <View key={`at-grid-item-${index}`} className={classNames(bodyClass, {
