...
import Taro, { makePhoneCall as _makePhoneCall, showModal as _showModal } from "@tarojs/taro-h5";
...
import { getUserIsAuth, checkMineAuthInfo } from "../../../utils/request/index";
import { useSelector } from "@tarojs/redux-h5";
import Auth from "../../../components/auth/index";
import './index.scss';
import Msg, { ShowActionModal } from "../../../utils/msg/index";
export default class CheckAuth extends Taro.Component {
  config = {
    navigationBarTitleText: '实名查询'
  };

  render() {
    const [tel, setTel] = Taro.useState('');
    ...
    <View className="checkauth-container">
      ...
      {show && model && <View className="checkauth-body">
        ...
        <View className="checkauth-res-item">
          ...
          {model.occupations.map((item, index) => <View className="checkauth-res-text" key={index}>{item}</View>)}
        </View>
        <View className="checkauth-res-item">
