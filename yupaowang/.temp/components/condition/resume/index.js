...
import Taro, { setStorageSync as _setStorageSync, getStorageSync as _getStorageSync } from "@tarojs/taro-h5";
...
class ResumeCondition extends Taro.Component {
  render() {
    ...
    // * 当前城市选择子级索引
    const [areaChildId, setAreaChildId] = Taro.useState('');
    // * 当前工种选择子级索引
    const [classifyChildId, setclassifyChildId] = Taro.useState('');
    // * 当前筛选数据索引
    ...
    // * 当前人员设置的索引
    const [memberIndex, setMemberIndex] = Taro.useState(0);
    // * 工种数据
    const [classify, setClassify] = Taro.useState([]);
    // * 数据筛选条件 推荐 最新
    const [resumeType, setResumeType] = Taro.useState([]);
    // * 数据筛选条件 推荐 最新
    const [memberType, setMemberType] = Taro.useState([]);
    // * 城市切换后子集列表回到顶部
    ...
    const sureClassifyCurrent = i => {
      ...
      let text = i ? classify[classifyIndex].children[i].name : classify[classifyIndex].name;
    ...
    const sureFilterCurrent = i => {
      setFilterIndex(i);
      let id = resumeType[i].type;
      setSearchData(ResumeFilterPickerKey, id, resumeType[i].name);
      closeDrawer();
    };
    // 选择人员构成
    const sureMemberCurrent = i => {
      setMemberIndex(i);
      let id = memberType[i].id.toString();
    ...
    return <Nerv.Fragment>
      ...
      <AtDrawer show={current === MemberPickerKey} mask onClose={() => closeDrawer()}>
        <View className="common-drawer-item">
          <ScrollView className="drawer-full-lists" scrollY>
            {memberType.map((item, index) => <View key={item.id} onClick={() => sureMemberCurrent(index)} className={classnames({
              'drawer-list-item overwords': true,
      ...
      <AtDrawer show={current === ResumeFilterPickerKey} mask onClose={() => closeDrawer()}>
        <View className="common-drawer-item">
          <ScrollView className="drawer-full-lists" scrollY>
            {resumeType.map((item, index) => <View key={item.type} onClick={() => sureFilterCurrent(index)} className={classnames({
              'drawer-list-item overwords': true,
...
export default Taro.memo(ResumeCondition);