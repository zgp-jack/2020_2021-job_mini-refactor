...
import Taro, { getStorageSync as _getStorageSync, showModal as _showModal, setStorageSync as _setStorageSync, clearStorageSync as _clearStorageSync } from "@tarojs/taro-h5";
...
import './index.scss';
export default class Distruction extends Taro.Component {
  config = {
  ...
  render() {
    ...
    // 所有省份
    const [are, setAre] = Taro.useState({
      areData: []
    });
    // 是否点击输入框
    const [clickInput, setClickInput] = Taro.useState(false);
    // 输入输入框时
    const [onInput, setOnInput] = Taro.useState(false);
    // 搜索的内容
    const [searchData, setSearchData] = Taro.useState({
      list: []
    });
    // 输入框内容
    const [inputVal, setInputVal] = Taro.useState('');
    // 搜索历史
    const [history, setHistory] = Taro.useState({
      historylist: []
    });
    // 点击参数
    ...
    Taro.useEffect(() => {
      // 获取搜索历史
      let searchItem = _getStorageSync(SearchList);
      if (searchItem) {
    ...
    const handleSeach = v => {
      ...
      const hash = {};
      const arr = history.historylist.reduce(function (item, next) {
      ...
      // 设置搜索历史
      _setStorageSync(SearchList, history.historylist);
    };
    // 清除历史
    ...
    return <View className="distruction">
      <View className={clickInput ? 'distruction-head-click' : 'distruction-head'}>
      <View className="distruction-top">
          <Input className="distruction-top-input" placeholder="输入城市名字进行搜素" onClick={handleInput} value={inputVal} onInput={e => handleOnInput(e)} />
        <View className="distruction-top-seach">搜索</View>
      </View>
        {onInput && (searchData.list.length > 0 ? <View className="distruction-top-seach-list-box">
          {searchData.list.map(v => <View onClick={() => handleSeach(v)} className="distruction-top-seach-list">{v.listName}</View>)}
        </View> : <View className="distruction-head-click-cityList">
            <View className="distruction-head-click-cityList-text">暂未找到相关城市</View>
          </View>)}
        {clickInput && !onInput && history.historylist.length > 0 && <View className="distruction-head-click-seachList">
          ...
          <View className="distruction-head-click-seachList-list-box">
            {history.historylist.map(v => <View key={v.id} className="distruction-head-click-seachList-list" onClick={() => handleSeach(v)}>{v.name}</View>)}
          </View>
        </View>}
      ...
      <View className="distruction-content">
        ...
        <View className="distruction-content-box">
          {data.item.map(v => <View onClick={() => {
            handleAllAre(v, 1);
          }} className={v.click ? 'distruction-content-box-list-click' : 'distruction-content-box-list'} key={v.id}>{v.name}
      ...
      <View className="distruction-content-are-content">
        {are.areData.map(v => <View className="distruction-content-are-content-box">
            ...
            <View className="distruction-content-are-content-box-list">
              {v.children.map((val, index) => <View onClick={() => handleAllAre(val, 0)} className={val.click ? 'distruction-content-are-content-box-list-item-click' : 'distruction-content-are-content-box-list-item'}>{index == 0 && v.index != 0 ? "全省置顶" : val.name}
